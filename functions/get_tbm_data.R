get_tbm_data <- function(tbm_orig) {
  
  # store units
  col_units <- tbm_orig[1, ]
  ...
  df <- cbind(df, 
              ChainageHead = chainage$ChainageHead,
              Ring = ring$Ring) 
  
  # remove rows with duplicated chainage
  n_occur <- data.frame(table(df$ChainageHead))
  duplic <- n_occur[n_occur$Freq > 1,]$Var1
  df <- df %>% filter(ChainageHead %!in% duplic)
